...
function ReviewsList() {
    const {loading,reviews,products,error,success,message}=useSelector(state=>state.admin)
    // console.log(reviews);
   ...
   const handleViewReviews=(productId)=>{
     ...
     const handleDeleteReivew=(productId,reviewId)=>{
        ...
        <tbody>
         {products.map((product,index)=>
             (  <tr key={product._id}>
                <td>{index+1}</td>
                <td>{product.name}</td>
                <td><img src={product.image[0].url} alt={`${product.name} image`}  className='product-image'/></td>
            ...
            <tbody>
             { reviews.map((review,index)=>
                ( <tr key={review._id}>
                    ...
                    <td>
<button className="action-btn delete-btn" onClick={()=>handleDeleteReivew(selectedProduct,review._id)}><Delete/></button>
